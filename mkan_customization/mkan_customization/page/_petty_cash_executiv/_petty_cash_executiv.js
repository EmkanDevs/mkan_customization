...
class PettyCashDashboard {
    constructor(page) {
        ...
        this._filterTimeout = null; // Used by queueDashboardRefresh
                ...
                <div class="pcd-card-body">
                    <div class="pcd-filters-grid">
                        ...
                        <div class="pcd-filter-group">
                            ...
                            <div id="pcd-filter-custodian-wrapper" class="pcd-link-field"></div>
                        </div>
                        <div class="pcd-filter-group">
                            <label>${__('Project')}</label>
                            <div id="pcd-filter-project-wrapper" class="pcd-link-field"></div>
                        </div>
                    </div>
                </div>
            </div> 
    ...
    // ── Link Fields Initialization ──

    initLinkFields() {
        // Custodian (Employee) Link Field
        this.custodian_control = frappe.ui.form.make_control({
            df: {
                fieldname: 'pcd_filter_custodian',
                label: __('Custodian (Employee)'),
                fieldtype: 'Link',
                options: 'Employee',
            ...
            parent: $('#pcd-filter-custodian-wrapper'),
            only_input: true,
        });
        this.custodian_control.make_input();
        
        // Bind change event to trigger dashboard refresh
        this.custodian_control.$input.on('change', () => {
            this.queueDashboardRefresh();
        });

        // Project Link Field
        this.project_control = frappe.ui.form.make_control({
            df: {
                fieldname: 'pcd_filter_project',
                label: __('Project'),
                fieldtype: 'Link',
                options: 'Project',
            ...
            parent: $('#pcd-filter-project-wrapper'),
            only_input: true,
        });
        this.project_control.make_input();

        // Bind change event to trigger dashboard refresh
        this.project_control.$input.on('change', () => {
            this.queueDashboardRefresh();
        });
    }
    ...
    getFilters() {
        return {
            ...
            custodian: this.custodian_control ? this.custodian_control.get_value() : '',
    ...
    loadFilterOptions() {
        frappe.call({
            ...
            // Note: Custodian and Project options are no longer loaded here; 
            // the Link fields handle their own dynamic search data.
        }).catch(() => {
    ...
    renderGridRows(rows) {
        ...
        function pcdDecodeEntities(str) {
            // Handles single- or double-encoded entities from the backend
            let previous;
        ...
        function pcdFormatItemsCell(rawItems) {
            if (!rawItems) return '—';
            const decoded = pcdDecodeEntities(rawItems);
        
            const tmp = document.createElement('div');
            tmp.innerHTML = decoded;
        
            const blocks = tmp.querySelectorAll('div, tr');
            const lines = [];
            if (blocks.length) {
                blocks.forEach(node => {
            ...
            return lines.map(l => `<div class="pcd-item-line">${pcdEscapeHtml(l)}</div>`).join('');
        }
        ...
        const parts = rows.map(row => `<tr>
            ...
            <td>${pcdFormatItemsCell(row.items)}</td> 
    ...
    bindEvents() {
        ...
        // Note: Custodian & Project are excluded here because their change events 
        // are bound directly inside the initLinkFields() method.
        $page.on('change', '#pcd-filter-company, #pcd-filter-from, #pcd-filter-to', () => this.queueDashboardRefresh());
    }

    queueDashboardRefresh() {
        clearTimeout(this._filterTimeout);
        this._filterTimeout = setTimeout(() => this.loadDashboard(), PCD_FILTER_DELAY);
    }
    ...
    resetFilters() {
        ...
        // Clear Link Fields
        if (this.custodian_control) this.custodian_control.set_value('');
