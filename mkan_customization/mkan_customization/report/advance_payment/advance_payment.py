...
def get_columns():
    return [
        ...
        {"fieldname": "total_advance", "label": _("Total Advance"), "fieldtype": "Currency", "width": 150},
...
def get_data(filters):
    ...
    query = f"""
        SELECT
			...
			COALESCE(si.total_advance, 0) as total_advance,
			CASE WHEN si.grand_total > 0
				THEN (COALESCE(si.total_advance, 0) / si.total) * 100
    ...
    for row in data:
        ...
        row["total_advance"] = flt(row["total_advance"], 2)
...
def get_number_cards(filters):
    ...
    summary_query = f"""
        SELECT
            ...
            SUM(COALESCE(si.total_advance, 0)) as total_advance_payment
    ...
    return [
        ...
        {
            "value": flt(result.get("total_advance_payment") or 0, 2),
            "label": _("Total Advance Payment"),
            "datatype": "Currency",
            "indicator": "Red"
        }
    ]
