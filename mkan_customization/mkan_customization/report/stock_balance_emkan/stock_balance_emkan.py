...
class StockBalanceReport:
	...
	def prepare_new_data(self):
		...
		# Get petty cash data
		petty_cash_data = self.get_petty_cash_data()
	...
	def get_petty_cash_data(self):
		...
		pi_items = frappe.db.sql("""
			...
			WHERE pi.docstatus = 1 AND pi.is_petty_cash = 1
		""", as_dict=True)
		...
		item_warehouse_map[group_by_key] = frappe._dict(
			{
				...
				"val_rate": 0.0,
				"is_petty_cash": False,  # Default value
			}
		)
	...
	for group_by_key in iwb_map:
		...
		for key, val in qty_dict.items():
			...
			if key in [
				...
				"company",
				"opening_fifo_queue",
				"is_petty_cash",  # Include petty cash field
			]:
				continue
