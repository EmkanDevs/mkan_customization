...
class StockBalanceReport:
	...
	def prepare_stock_ledger_entries(self):
		...
		purchase_receipt = frappe.qb.DocType("Purchase Receipt")  # Added Purchase Receipt table
		...
		query = (
			...
			.on((sle.voucher_type == "Stock Entry") & (sle.voucher_no == stock_entry.name))
			.left_join(purchase_receipt)  # Join with Purchase Receipt
			.on((sle.voucher_type == "Purchase Receipt") & (sle.voucher_no == purchase_receipt.name))
			.select(
				sle.item_code,
				sle.warehouse,
				sle.posting_date,
				...
				purchase_receipt.is_petty_cash,  # Added to fetch petty cash flag
			)
