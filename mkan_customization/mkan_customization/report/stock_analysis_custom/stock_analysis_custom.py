...
def get_columns(filters):
	...
	for _dummy, end_date in ranges:
		...
		columns.append({"label": _(period), "fieldname": scrub(period), "fieldtype": "Data", "width": 160})
...
def get_periodic_data(entries, filters):
    expected_ranges = get_period_date_ranges(filters)
    expected_periods = [get_period(end_date, filters) for _start, end_date in expected_ranges]

    periodic_data = {}

    entries = sorted(entries, key=lambda d: (d.posting_date, getattr(d, "posting_datetime", None) or d.posting_date))
    ...
    for d in entries:
        ...
        qty = d.qty_after_transaction or 0.0
        val = d.stock_value or 0.0
        periodic_data.setdefault(item, {})
        running_totals.setdefault(item, {})
        running_totals[item][warehouse] = {"qty": qty, "val": val}
        period = get_period(d.posting_date, filters)
        total_qty = sum(v["qty"] for v in running_totals[item].values())
        total_val = sum(v["val"] for v in running_totals[item].values())

        periodic_data[item][period] = {"total_qty": total_qty, "total_val": total_val}
    for item, item_data in periodic_data.items():
        last_known = {"total_qty": 0.0, "total_val": 0.0}
        for period in expected_periods:
            if period in item_data:
                last_known = item_data[period]
            else:
                item_data[period] = last_known.copy()

    for item, item_data in periodic_data.items():
        for period in expected_periods:
            t = item_data[period]
            item_data[period] = f"Qty: {t['total_qty']:.2f} | Value: {flt(t['total_val'], 2):,.0f}"
...
def get_data(filters):
	...
	for _dummy, item_data in item_details.items():
		...
		previous_period_value = ""  # keep last string value
		for start_date, end_date in ranges:
			period = get_period(end_date, filters)
			period_data = periodic_data.get(item_data.name, {}).get(period)

			if period_data:
				# period_data is now already a single string like "Qty: 0.01 | Value: 100.00"
				previous_period_value = period_data
